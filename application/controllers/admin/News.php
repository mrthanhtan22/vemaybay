...
Class News extends MY_Controller
{
    ...
    function index()
    {
        ...
        $config['per_page']   = 4;//so luong bài viết hien thi tren 1 trang
        ...
        //lay the loai bai viet
        $catalog_id = $this->input->get('catalog');
        $catalog_id = intval($catalog_id);
            
        if ($catalog_id) {
        $input['where']['catalog_news_id'] = $catalog_id;
        }

        /*lay danh muc san pham*/
            $this->load->model('catalog_news_model');
            $catalogs = $this->catalog_news_model->get_list();
        ...
        if($this->input->post())
        {
            ...
            if($this->form_validation->run())
            {
                ...
                $data = array(
                    ...
                    'created'    => now(),
                    'catalog_news_id' => $this->input->post('catalog')
                ); 
                //them moi vao csdl
